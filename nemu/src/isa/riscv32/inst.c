...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (((int32_t)src1) >> (imm & rmask))); 
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << (imm & rmask)); 
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> (imm & rmask)); 
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (((int32_t)src1) >> (src2 & rmask))); 
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(rd) = src1 >> (src2 & rmask)); 
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(rd) = src1 << (src2 & rmask));   
  ...
  //目前的实现只识别yield和syscall   //bool success;s->dnpc = (isa_raise_intr(isa_reg_str2val("a7", &success), s->pc));
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N,  word_t a7 = gpr(17); word_t NO; NO=(a7==-1)?1:2; s->dnpc=isa_raise_intr(NO, s->pc) );//跳转到异常入口地址
