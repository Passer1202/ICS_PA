...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << (imm&31));
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> (imm&31)); 
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (word_t)((int32_t)src1 >> (imm&31)));
  INSTPAT("??????? ????? ????? 110 ????? 00000 11", lwu    , I, R(rd) = Mr(src1 + imm, 4));
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm);///////
  /*R*/
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra   , R, R(rd) = (word_t)((int32_t)src1 >> (src2&31)));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div   , R,  R(rd) =(int32_t)src1/(int32_t)src2);//
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = src1 / src2 );
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = src1 * src2);//
  //INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh    , R, int64_t temp_m=((int64_t)SEXT(src1,32) * (int64_t)SEXT(src2,32));R(rd) =BITS(temp_m,63,32));
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh    , R, R(rd)=((SEXT(src1,32)*SEXT(src2,32))>>32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu    , R, uint64_t temp_m=((uint64_t)SEXT(src1,32) * (uint64_t)SEXT(src2,32));R(rd) =BITS(temp_m,63,32));
